...
fun ProductFilterScreen(
    ...
    onBackClick: () -> Unit,
    onNavigateToProduct: (List<String>?, List<String>?) -> Unit,
    ...
    Scaffold(
        floatingActionButton = {
            SaveFilterFab(onClick = {
                ...
                val skinType: List<String>? = selectedFilters["skinType"]
                val category: List<String>? = selectedFilters["category"]
        ...
        content = { innerPadding ->
            Column(
                ...
                when (filterState) {
                    ...
                    is Result.Success -> {
                        val filter: ProductFilter = (filterState as Result.Success<ProductFilter>).data
                        FilterSection("skinType", filter.skinType, viewModel)
                        Spacer(modifier = Modifier.height(24.dp))
                        FilterSection("category", filter.category, viewModel)
                    }
                    is Result.Error -> {
...
@Composable
fun FilterSection(category: String, options: List<String?>?, viewModel: ProductFilterViewModel) {
    val title =
        if (category == "skinType") "Jenis Kulit"
        else "Kategori"
